			     ...
			     char *uuid_str_out, const int32_t uuid_str_out_size)
{
	...
	snprintf(uuid_str_out, APP_JWT_UUID_V4_STR_LEN, "%.8s-%.4s-%.4s-%.4s-%.4s%.8s",
		temp_str[0], temp_str[1] + 0, temp_str[1] + 4, temp_str[2] + 0,
		temp_str[2] + 4, temp_str[3]);
...
static char *jwt_header_create(struct app_jwt_data *const jwt)
{
	...
	if (jwt->add_keyid_to_header) {
		...
		for (uint32_t i = 0; i < 8; i++) {
			printed_bytes +=
				snprintf((sha256_str + printed_bytes), 9, "%08x",
...
static char *convert_str_to_b64_url(const char *const str)
{
	...
	if (b64_out) {
		int err = base64_encode(b64_out, b64_sz, &b64_sz, str,
			strnlen(str, JWT_STR_MAX_SZ));
			     ...
			     size_t sig_sz)
{
	...
	err = sign_message(key_id, jwt, strnlen(jwt, JWT_STR_MAX_SZ), sig_raw,
			ECDSA_SHA_256_SIG_SZ, &o_len);
	if (err) {
				...
				char *const jwt_buf, size_t jwt_sz)
{
	...
	/* '.' and null-terminator */
	size_t final_sz = strnlen(unsigned_jwt, JWT_STR_MAX_SZ) + 1 +
				strnlen(sig, ECDSA_SHA_256_SIG_SZ) + 1;
