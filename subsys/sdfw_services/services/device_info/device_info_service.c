...
int ssf_device_info_get_uuid(uint32_t *uuid_words, const size_t uuid_words_count)
{
	...
	if ((NULL != uuid_words) && (uuid_words_count >= UUID_BYTES_LENGTH)) {
		...
		struct device_info_req read_request = {
			.device_info_req_msg_choice = device_info_req_msg_read_req_m_c,
			.device_info_req_msg_read_req_m = uuid_read_request};

		struct device_info_resp read_response;
		struct read_resp uuid_read_response;
		...
		uuid_read_response = read_response.device_info_resp_msg_read_resp_m;

		if (read_response.device_info_resp_msg_choice ==
		    device_info_resp_msg_read_resp_m_c) {
			if (entity_UUID_c == uuid_read_response.read_resp_target.entity_choice) {
				ret = read_response.device_info_resp_msg_read_resp_m
					      .read_resp_status.stat_choice;
		...
		uuid_words_len = uuid_read_response.read_resp_data_uint_count;
