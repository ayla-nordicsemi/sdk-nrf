		...
		zcbor_state_t *state, struct device_info_req *result)
{
	...
	&& ((zcbor_union_start_code(state) && (int_res = ((((decode_read_req(state, (&(*result).device_info_req_msg_read_req_m)))) && (((*result).device_info_req_msg_choice = device_info_req_msg_read_req_m_c), true))
	|| (zcbor_union_elem_code(state) && (((decode_write_req(state, (&(*result).device_info_req_msg_write_req_m)))) && (((*result).device_info_req_msg_choice = device_info_req_msg_write_req_m_c), true)))), zcbor_union_end_code(state), int_res)))) || (zcbor_list_map_end_force_decode(state), false)) && zcbor_list_end_decode(state))));
		...
		zcbor_state_t *state, struct device_info_resp *result)
{
	...
	bool res = (((zcbor_list_start_decode(state) && ((((zcbor_uint32_expect(state, (3))))
	&& ((zcbor_union_start_code(state) && (int_res = ((((decode_read_resp(state, (&(*result).device_info_resp_msg_read_resp_m)))) && (((*result).device_info_resp_msg_choice = device_info_resp_msg_read_resp_m_c), true))
	|| (zcbor_union_elem_code(state) && (((decode_write_resp(state, (&(*result).device_info_resp_msg_write_resp_m)))) && (((*result).device_info_resp_msg_choice = device_info_resp_msg_write_resp_m_c), true)))), zcbor_union_end_code(state), int_res)))) || (zcbor_list_map_end_force_decode(state), false)) && zcbor_list_end_decode(state))));
